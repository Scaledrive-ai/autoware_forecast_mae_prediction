...
def collate_fn(batch):
    ...
    if "origin_all" in batch[0]:
        data["origin_all"] = pad_sequence([b["origin_all"] for b in batch], batch_first=True)
    if "theta_all" in batch[0]:
        data["theta_all"] = pad_sequence([b["theta_all"] for b in batch], batch_first=True)
