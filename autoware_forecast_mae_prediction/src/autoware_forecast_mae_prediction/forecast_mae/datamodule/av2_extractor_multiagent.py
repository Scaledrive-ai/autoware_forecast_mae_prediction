...
class Av2ExtractorMultiAgent:
    ...
    def process(self, raw_path: str, agent_id: str = None):
        ...
        actor_ids = list(cur_df["track_id"].unique()) # creating list of actor id for actor with 50 timestep history
        ...
        cur_head = torch.from_numpy(cur_df["heading"].values).float()
        ...
        actor_ids = [aid for i, aid in enumerate(actor_ids) if not out_of_range[i]] # actors with complete 50 timestep and within range
        av_idx = actor_ids.index(agent_id)
        scored_agents_mask = scored_agents_mask[~out_of_range]
        num_nodes = len(actor_ids)

        keep_mask = np.array([not out for out in out_of_range])  # shape (N,)
        out_of_range = out_of_range[keep_mask]
        cur_pos = cur_pos[keep_mask]  # positions of valid agents
        cur_head = cur_head[keep_mask]  # heading of valid agents
        print("check 1: saving position and heading") 

        df = df[df["track_id"].isin(actor_ids)] # updaing df to only keep actor with complete 50 timestep and within range
        ...
        if self.remove_outlier_actors:
            ...
            cur_pos = cur_pos[valid_actor_mask]
            cur_head = cur_head[valid_actor_mask]
            print("check 2: saving position and heading")
        ...
        return {
            ...
            "theta": theta,
            "origin_all": cur_pos,    # (num_nodes, 2)
            "theta_all":  cur_head,   # (num_nodes,)
