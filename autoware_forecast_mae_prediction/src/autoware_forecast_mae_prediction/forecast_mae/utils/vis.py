...
def visualize_scenario(
    ...
    cur_plot_bounds = _plot_actor_tracks(ax, scenario, timestep, id_to_cat=id_to_cat)
    plot_bounds = cur_plot_bounds
    num_agents = prediction.shape[1]
    colors = plt.cm.get_cmap("tab10")(np.linspace(0,1,num_agents))

    if prediction is not None and plot_bounds is not None:
        for a in range(num_agents):
            pred_agent = prediction[:2, a, :, :]
            _scatter_polylines(
                pred_agent,
                ax,
                color=colors[a],
...
def _plot_actor_tracks(
    ...
    for track in scenario.tracks:
        ...
        cat = id_to_cat.get(track.track_id, None) if id_to_cat else None
        ...
        if track.category == TrackCategory.FOCAL_TRACK and len(future_trajectory) > 0:
            _scatter_polylines(
                [future_trajectory],
                cmap="spring",
                linewidth=6,
